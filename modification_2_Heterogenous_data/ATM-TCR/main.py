...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Prediction of TCR binding to peptide-MHC complexes"
    )

    parser.add_argument("--infile", type=str, help="Input file for training")
    parser.add_argument(
        "--indepfile",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--cuda", type=str2bool, default=True, help="enable cuda"
    )
    ...
    parser.add_argument(
        "--save_model", type=str2bool, default=True, help="save model"
    )
    parser.add_argument(
        "--model", type=str, default="attention", help="Model to import"
    )
    parser.add_argument(
        "--drop_rate", type=float, default=0.25, help="dropout rate"
    )
    parser.add_argument(
        "--lin_size",
        type=int,
    ...
    parser.add_argument(
        "--padding", type=str, default="mid", help="front, end, mid, alignment"
    )
    parser.add_argument(
        "--heads", type=int, default=5, help="Multihead attention head"
    )
    parser.add_argument(
        "--max_len_tcr",
        type=int,
    ...
    assert (
        args.idx_test_fold < args.n_fold
    ), "--idx_test_fold should be smaller than --n_fold"
    assert (
        args.idx_val_fold < args.n_fold
    ), "--idx_val_fold should be smaller than --n_fold"
    assert (
        args.idx_val_fold != args.idx_test_fold
    ), "--idx_val_fold and --idx_test_fold should not be equal to each other"
    ...
    # Define dataloader
    train_loader = define_dataloader(
        x_pep[idx_train],
        x_tcr[idx_train],
        y[idx_train],
        args.max_len_pep,
    ...
    if args.indepfile is not None:
        x_indep_pep, x_indep_tcr, y_indep = data_io_tf.read_pTCR(
            args.indepfile
        )
        y_indep = np.array(y_indep)
        indep_loader = define_dataloader(
            x_indep_pep,
            x_indep_tcr,
            y_indep,
            maxlen_pep=train_loader["pep_length"],
    ...
    if args.mode == "train":
        wf_open = open(
            "result/perf_"
            + os.path.splitext(os.path.basename(args.model_name))[0]
            + ".csv",
            "w",
        )
        wf_colnames = [
        ...
        for epoch in range(1, args.epoch + 1):
            ...
            if epoch % PRINT_EVERY_EPOCH == 0:
                ...
                print_performance(
                    perf_test, printif=False, writeif=True, wf=wf
                )

            # Check for early stopping
            lossArray.append(perf_test["loss"])
            average_loss_change = (
                sum(np.abs(np.diff(lossArray))) / lossArraySize
            )
            if (
                epoch > args.min_epoch
                and average_loss_change < 10
                and args.early_stop
            ):
        ...
        if args.indepfile is not None:
            ...
            wf_open = open(
                ...
                + os.path.splitext(os.path.basename(args.model_name))[0]
                + "_"
                + os.path.basename(args.indepfile),
                "w",
            )
            wf = csv.DictWriter(wf_open, wf_colnames, delimiter="\t")
