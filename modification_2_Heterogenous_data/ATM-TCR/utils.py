...
def write_blackbox_output_batchiter(
    loader, model, wf, device="cpu", ifscore=True
):
...
def get_performance(score, label):
    ...
    auc = (
        roc_auc_score(np.array(label), np.array(score))
        if len(np.unique(np.array(label))) != 1
        else -1
    )

    ndigits = 4
    performance = {
        "accuracy": round(accuracy, ndigits),
        "precision1": round(precision1, ndigits),
        "precision0": round(precision0, ndigits),
        "recall1": round(recall1, ndigits),
        "recall0": round(recall0, ndigits),
        "f1macro": round(f1macro, ndigits),
        "f1micro": round(f1micro, ndigits),
...
def print_performance(
    perf, printif=True, writeif=False, boardif=False, **kargs
):
    """
    print classification performance for binary task
    ...
    if boardif:
        ...
        for mea in measures:
            kargs["tbf"].add_scalars(
                main_tag="performance/{}".format(mea),
...
def check_model_name(model_name, file_path="models"):
    ...
    if model_name in os.listdir(file_path):
        ...
        sys.stdout.write(
            "The file {} already exists. Do you want to overwrite it? [yes/no]".format(
                model_name
            )
        )
        choice = input().lower()
...
def get_physchem_properties_batchiter(loader, lenpep, lentcr, device="cpu"):
    ...
    for batch in loader:
        ...
        get_physchem_properties(
            X_pep[np.where(y_true == 1.0)].tolist(),
...
def get_physchem_properties(
    pep_batch, tcr_batch, max_len_pep, max_len_tcr, exclude
):
    ...
    if align:
        ...
        for seq in seq_list:
            ...
            if max_len is None:
                ...
                num = [mapping[seq[i]] for i in range(len(seq))] + [
                    mapping["<pad>"] for _ in range(max_len - len(seq))
                ]
            else:
        ...
        for num in num_list:
            ...
            if max_len is None or max_len > len(num):
                seq = [
                    mapping[num[i]]
                    for i in range(len(num))
                    if mapping[num[i]] not in exclude
                ]
            else:
                seq = [
                    mapping[num[i]]
