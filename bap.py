...
class TCRDataset(Dataset):
    def __init__(self, csv_file, transform=None):
        ...
        # Extract epitope, TCR sequence, and labels
        self.epitopes = self.data.iloc[:, 0].values  # First column
        self.tcrs = self.data.iloc[:, 1].values  # Second column
        self.labels = self.data.iloc[:, 2].values  # Third column
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            epitope = self.transform(epitope)
            tcr = self.transform(tcr)

        # Convert to tensors
        label = torch.tensor(label, dtype=torch.float32)  # Binary label
        return epitope, tcr, label


def get_inputs(embedding_type):
    if embedding_type == "catELMo":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/catELMo_combined.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)
    elif embedding_type == "blosum62":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/BLOSUM62.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)

    elif embedding_type == "blosum62_22_24":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/BLOSUM62_20_22.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)

    elif embedding_type == "SeqVec":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/SeqVec.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)
    elif embedding_type == "ProtBert":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/ProtBert.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)
    elif embedding_type == "catBert":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/catBert.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)
    elif embedding_type == "Doc2Vec":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/Doc2Vec.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)
    elif embedding_type == "catELMo_finetuned":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/catELMo_finetuned.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)

    elif embedding_type == "catBert_768_12_layers_mlm_nsp":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/Small_Bert_mlm_nsp.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)

    elif embedding_type == "catBert_768_12_layers_mlm":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/Small_Bert_mlm.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)

    elif embedding_type == "TCRbert":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/TCRBert_mlm_12_layers.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)

    elif embedding_type == "catBert_768_2_layers_mlm":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/Small_Bert_mlm_2_layers.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)

    elif embedding_type == "catELMo_4_layers_1024":
        dat = pd.read_pickle(
            "/mnt/disk07/user/pzhang84/data/tcr_repertoires_healthy_samples/combined_dataset_repTCRs/catELMo_4_layers_1024.pkl"
        )
    #         dat = dat.sample(frac=1).reset_index(drop=True)
...
def load_data_split(dat, split_type, seed):
    ...
    if split_type == "random":
        ...
        if idx_val_fold < 0:
            ...
            idx_test = [
                index
                for index, pep in enumerate(x_pep)
                if pep in test_peptides
            ]
            ...
            idx_test_remove = [
                index
                for index, pep in enumerate(x_pep)
                if pep in test_remove_peptides
            ]
            idx_train = list(
                set(range(len(x_pep)))
                .difference(set(idx_test))
        ...
        if idx_val_fold < 0:
            ...
            idx_test = [
                index for index, tcr in enumerate(x_tcr) if tcr in test_tcrs
            ]
            ...
            idx_test_remove = [
                index
                for index, tcr in enumerate(x_tcr)
                if tcr in test_remove_tcrs
            ]
            idx_train = list(
                set(range(len(x_tcr)))
                .difference(set(idx_test))
...
def train_(
    embedding_name, X1_train, X2_train, y_train, X1_test, X2_test, y_test
):
...
def main(embedding, split, fraction, seed, gpu):
    ...
    tr_dat = dat
    tr_dat = dat.sample(
        frac=fraction, replace=True, random_state=seed
    ).reset_index(
        drop=True
    )  # comment this out if no fraction used
    (
        X1_train,
        X2_train,
        y_train,
